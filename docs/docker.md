...
Run the development containers of Gobierno Autonomo Municipal De Laja with Docker and Docker Compose utility. Docker allows you to compose 
...
class Config {
    ...
    const DB_HOST       = 'gamlaja-database:3306';
...
You can remove the docker containers with `docker rm gamlaja-server gamlaja-database`. 

You can remove the server image with `docker rmi gamlaja-server:v1`.
